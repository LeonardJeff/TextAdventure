...
pushtext("...", speed = 0.2, cutscenemode= True)
pushtext("... Hey. Buddy.","\n???",0.025)
pushtext("...", speed = 0.2, cutscenemode= True)
pushtext("... Hey, BUDDY.", "\n???",0.02)
...
pushtext("What's your name. I can't be having new-hires sleeping on the job.", "???", 0.025)
print("Enter your name:")
name = input()
if name == "":
    name = "Gabe"
    players.player.setName("Gabe")
    pushtext("... So you're the silent type?","???", 0.02)
    pushtext("The man looks down at his clipboard.")
    pushtext("...well bud, the only name I don't recognize on here is Gabe. So you must be Gabe.","???", 0.025)
...
rooms.r2.addscenery(scenerys.beehive)
...
pushtext(f"{players.player.name}? {players.player.name}... I think I've heard my daughter mention your name before.","???", 0.025)
pushtext("Anyway, my name is Frank, but you can call me sir. I've been working construction for 12 years and I've never had a worse first impression for a new-hire of mine.","Frank", 0.02)
pushtext(f"Well {players.player.name}, If you actually want to get paid for helpin us with the flour-mill today, start by fetching me some planks from the drop-off point to the south of here.","Frank", 0.02)
...
pushtext("Ahhhhhhhh!!!!", speed=0.03, cutscenemode = True) 
time.sleep(0.8)
pushtext("\nYou hear screams from the windmill north of you.", speed=0.03, cutscenemode = True)
time.sleep(0.8)
...
pushtext(f"You feel butterflies in your stomach as she pulls you further onto land.", speed = 0.025)
pushtext(f"{players.player.name}! I thought you were a goner! That forest spirit sent you flying!","Lily", 0.025)
pushtext(f"As Lily lays you on your side, you cough up a small minnow.", speed = 0.025)
pushtext(f"Despite coughing up a live fish, you still feel a weird sensation from deep within your stomach.", speed = 0.025)
pushtext(f"Lily waits for you to say something - but quickly remembers that you're not much of a talker.", speed = 0.025)
pushtext(f"You know, it was a pretty dumb idea to attack that forest spirit. When they get angry, there's not much that can calm them down.","Lily", 0.025)
pushtext(f"... What? You think it was a demon? Nonsense, what would a demon be doing at a construction site?","Lily", 0.025)
pushtext(f"Well, you seem to be in good shape despite being flung this far away from the flour mill.","Lily", 0.025)
pushtext(f"The sun is going to set soon, if you'd like a place to stay tonight, I'm sure my parents wouldn't mind if you stayed at our house for the night.","Lily", 0.02)
pushtext(f"Our house is a little bit north of here. Lets get going before goblins and other animals start to come out.","Lily", 0.02)
pushtext(f"I'll meet you there!","Lily", 0.02)
pushtext(f"Lily runs off - leaving you to fend for yourself.")

rooms.r6.addscenery(scenerys.coatrack)
...
pushtext(f"Oh uh {players.player.name}. Hi. Glad to see you made it out of that demon fight in one peice.", speaker = "Frank", speed = 0.025)
pushtext(f"Once I heard the screaming from the flour mill, like any good boss would do, I carried two of my men on my back and booked it out of there.", speaker = "Frank", speed = 0.025)
pushtext(f"Oh didja now sweetie?", speaker = "Peggy", speed = 0.03)
pushtext(f"I'm pretty sure you made it back home before I heard that loud crash at the lake. Where'd you bring those two guys??", speaker = "Peggy", speed = 0.025)
pushtext(f"Oh uh, I dropped them off at the infirmary in Galawyn before I came back. They were really hurt, so I made sure to get them there quick. Like any good boss would do.", speaker = "Frank", speed = 0.02)
pushtext(f"I would've come back for you {players.player.name}, but I knew Lily would make sure you made it out safe. She had her eyes on you the whole -", speaker = "Frank", speed = 0.025)
pushtext([f"Frank!", "Shut it!"], speaker = "Peggy", speed = 0.02)
pushtext(f"Well ah, {players.player.name}, if you'd like your pay for today, I'm afraid you'll need to go to the bank in Galawyn to cash this promissory note in.", speaker = "Frank", speed = 0.025)
pushtext("Frank hands you an official looking peice of paper.")
pushtext([f"You're going to make {players.player.name} go all the way to Galawyn!?", f"The whole city has been on high-alert for the past couple of days, {players.player.name} wouldn't even be able to get past the guards!"], speaker = "Peggy", speed = 0.025)
pushtext(f"Nonesense. All these demon sightings have the city on high alert for demons. Not kids.", speaker = "Frank", speed = 0.025)
...
"Be careful though sweetie. We have that gate for a reason. All sorts of scary wildlife is out there."], speaker = "Peggy", speed = 0.025)
...
while rooms.r7.entered == False:        #Spawn wandering goblin once player enters room 7
    players.player.location.displayroom(players.player)
pushtext("You get in the way of me picking my color plants!", "Wandering Goblin", 0.025)
