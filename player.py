...
class Player:
    ...
    def getArmorRating(self):
        total = 0
        if self.headslot != None:
            total += self.headslot.armorrating
        if self.bodyslot != None:
            total += self.bodyslot.armorrating
        if self.bootslot != None:
            total += self.bootslot.armorrating
        if self.ringslot != None:
            total += self.ringslot.armorrating
        if self.weapon != None:
    ...
    def calcdamage(self, enemydefense, enemyspeed):
        ...
        if self.weapon:         #refine logic for this- add armor bonuses to attack if applicable
